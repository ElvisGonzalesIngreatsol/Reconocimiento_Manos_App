...
class MainActivity : AppCompatActivity() {
    ...
    val BASE_URL = "http://192.168.0.107:8000/"
    ...
    private fun ejecutarInicializacionCameraX() {
        ...
        // Reiniciar acumuladores de recorrido al arrancar de cero la captura
        manosRegistradasIds.clear()
        maxManosEnUnInstante360 = 0
        ...
        cameraProviderFuture.addListener({
            ...
            imageAnalysis.setAnalyzer(ContextCompat.getMainExecutor(this)) { imageProxy ->
                if (!estaProcesandoFrameEnVivo && modoSeleccionado == 2) {
                                    ...
                                    if (response.isSuccessful && response.body() != null && modoSeleccionado == 2) {
                                        ...
                                        if (!apiResult.error) {
                                            resatadorView.setDetecciones(
                                                apiResult.detecciones,
                                                bitmap.width,
                                                bitmap.height,
                                                previewViewCameraX
                                            )

                                            // LÓGICA DE FIJACIÓN DE PICO MÁXIMO
                                            // Si las manos detectadas ahora superan el récord anterior, congelamos el nuevo máximo
                                            if (apiResult.manos > maxManosEnUnInstante360) {
                                                maxManosEnUnInstante360 = apiResult.manos
                                            }

                                            // Extraer y guardar los IDs únicos que devuelve el servidor
                                            for (det in apiResult.detecciones) {
                                                det.id?.let { idMano ->
                                                    manosRegistradasIds.add(idMano)
                                                }
                                            }

                                            // Siempre mostramos el récord más alto guardado en el subtítulo
                                            txtContadorFotos.text = "Manos en este instante (Máx): $maxManosEnUnInstante360"

                                            // Desplegar de forma permanente el pico histórico y el tamaño del set de IDs sin repetir
    ...
    // Muestra persistentemente el pico máximo fijado y el recorrido sin duplicados
